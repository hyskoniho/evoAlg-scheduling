...
def quantidade_aulas(
        horario: str,
        quantidade: int = 1,
        requisitos: dict = REQUISITOS) -> int:
    ...
    nota_quantidade: int = sum(  # Somatório
        abs(  # Valor absoluto para obter a diferença entre a quantidade de aulas esperada e a quantidade de aulas no horário
            # Quantidade esperada daquela matéria - Quantidade de aulas daquela
            # matéria no horário
            (requisitos[materia] * quantidade) - c.get(materia, 0)
        )
        # Aplica a operação para cada matéria, e soma o resultado
        for materia in requisitos
    )
...
def validar_restricoes(horario: str) -> int:
    ...
    # TODO:  Somar a distância entre o horário esperado x horário anotado pois
    # incentiva mais a aproximação do horário esperado
    ...
    for turma in separar_turmas(horario):
        ...
        for d, dia in enumerate(separar_dias(turma)):
            ...
            if 'E' in dia and dia.find(
                    'E', 0, 2) <= 1 and dia.find(
                    'E', 0, 2) != -1:
                nota_restricoes += 1
            ...
            # Verifica se o dia de Artes é segunda-feira, terça-feira ou
            # sexta-feira
            if 'A' in dia and d not in [0, 1, 4]:
                nota_restricoes += sum(1 for aula in dia if aula == 'A')

            # Verifica se o dia de Música é segunda-feira, terça-feira ou
            # sexta-feira
...
def fitting(horario: str, requisitos: dict = REQUISITOS) -> int:
    ...
    return round(
        (quantidade_aulas(
            ...
            4,
            requisitos) +
            validar_restricoes(horario) +
            validar_sobreposicoes(
            separar_turmas(horario))),
        2)
